...
def main() -> int:
    ...
    ap.add_argument(
        "--strategy",
        choices=["ma_crossover", "mean_reversion", "breakout"],
    ...
    dsr = deflated_sharpe_for_report(
        ...
        ledger_path=ledger,
        count_scope="total",
        expected_trials=args.campaign_trials,
    )
    ...
    if not unversehrt.ok:
        print("FEHLGESCHLAGEN — Versuchsregister beschaedigt:", file=sys.stderr)
        for problem in unversehrt.problems:
            print(f"  {problem}", file=sys.stderr)
        print(
            "Eine Deflation gegen ein beschaedigtes Register belegt nichts.",
            file=sys.stderr,
        )
        return 1
    ...
    stressed = run_backtest(
        oos,
        factory(),
        stressed_spec(spec, prereg.cost_stress_multiplier),
        strategy_id="stress",
        seed=0,
        data_checksum="",
        code_commit="",
