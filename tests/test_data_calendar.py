...
def test_weekday_holiday_not_counted_as_expected() -> None:
    ...
    common = dict(instrument="EURUSD", timeframe="D1", start=start, end=end,
    ...
    mit = assess_bars(bars, holidays=frozenset({date(2025, 12, 25)}),
...
def test_intraday_sunday_bars_not_outside_session() -> None:
    ...
    report = assess_bars(
        bars, instrument="EURUSD", timeframe="H1", start=base,
        end=base + timedelta(hours=3), session_predicate=FxSession(),
    )
...
def test_weekday_session_would_flag_sunday_but_fx_does_not() -> None:
    ...
    common = dict(instrument="EURUSD", timeframe="H1", start=base,
